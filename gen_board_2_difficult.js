...
function init() {
	...
	ws.onmessage = function (e) {
		...
		if (received_data == 'D1'){
			dif = document.getElementById('P2_turn2')
			dif.innerHTML = '<div class="balloon"><p><b>難しい！</b></p></div>';
			setTimeout(erace_dif, 1500, dif);
			return
		}
		...
		p.then((resolve,reject) =>{
			...
			if(turn % PLAYERNUMBER == 0){//change
				...
				dif = document.getElementById('P1_turn2')
				dif.innerHTML = '<button type="button" class="btn" onclick = "(difficult_click())" >難しい！</button>'
			}else{
				buttons_erase();
				dif = document.getElementById('P1_turn2')
				dif.innerHTML = ''
...
function hands_generate(received_data) {
	...
	if (received_data[0] == 2) {
		for (var i = 0; i < 5; i++) {
			...
			if (received_data[130] == 1 & received_data[131] == 't'){
				continue
			}
...
function difficult_click(){
	console.log('D2')
	var button = document.getElementById("P1_turn2");
	for (h_i = 0; h_i < 5; h_i++) {
		P2_buttons[h_i].innerHTML = '<input type = "button" id = "P1_' + String(h_i + 1) + '_play" value = "プレイ" onclick = "gen_message(this)" disabled> <input type = "button" id = "P1_' + String(h_i + 1) + '_discard" value = " 廃棄 " onclick = "gen_message(this)" disabled>';
		P1_buttons[h_i].innerHTML = '<input type = "button" id = "P2_' + String(h_i + 1) + '_color" value = "  色  " onclick = "gen_message(this)" disabled> <input type = "button" id = "P2_' + String(h_i + 1) + '_number" value = " 数字 " onclick = "gen_message(this)" disabled>';
	}
	button.innerHTML = '<div class="balloon"><p><b>難しい！</b></p></div>'
	setTimeout(function() {
		for (h_i = 0; h_i < 5; h_i++) {
			P2_buttons[h_i].innerHTML = '<input type = "button" id = "P2_' + String(h_i + 1) + '_play" value = "プレイ" onclick = "gen_message(this)"> <input type = "button" id = "P2_' + String(h_i + 1) + '_discard" value = " 廃棄 " onclick = "gen_message(this)">';
			if (b_token == "0") {
		...
		button.innerHTML = '<button type="button" class="btn" onclick = "(difficult_click())">難しい！</button>'
	}, 1500);
	ws.send("D2");
}

function erace_dif(dif){
	dif.innerHTML = '';
}
