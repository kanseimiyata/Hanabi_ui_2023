...
var playername = { 1:'あなた', 2:'相手'}//change_2
	...
	ws.onmessage = function (e) {
		...
		if (received_data[0] == 'O'){
			if (received_data[1] == '1'){
				...
				document.getElementById('P1_'+received_data[3]).innerHTML = '<img src="pict/' + hint +'.png" width = "120" height = "150">'
				document.getElementById('P1N_'+received_data[3]).innerHTML = String(5-(received_data.match(/0/g)|| []).length)
					...
					let tenmetsu = setInterval(function(){
						if (MyFlg%2 == 0){
							for (var i = 0; i < 4; i++) {
								...
								if (place != '0'){
									document.getElementById('P1_'+place).innerHTML = '<img src="pict/' + hint +'.png" width = "120" height = "150">'
								}
							}
						}else{
							for (var i = 0; i < 4; i++) {
								place = received_data_O[i+3]
								if (place != '0'){
									document.getElementById('P1_'+place).innerHTML = ' <img src="pict/Hanabi_default.png" width = "120" height = "150">';
								}
							}
						}
						if(MyFlg > 5){
		...
		if (received_data[130] == '1'){
			for (var i = 0; i < 5; i++) {
				document.getElementById('P1N_'+String(i+1)).innerHTML = '　'
			}
		}
		...
		p.then((resolve,reject) =>{
			...
			if(turn % PLAYERNUMBER == received_data[0] || turn == 0){//change
			...
			// if(received_data[134] != null & received_data[131] == 't' & received_data[132] == '1'){
			// 	MyFlg = 0;
	...
	for (h_i = 0; h_i < 5; h_i++) {
		if (received_data.slice(9).includes(String(h_i+1))){
			if (received_data[8] == 'N'){
				P2_buttons[h_i].innerHTML = '<input type = "button" id = "P2_' + String(h_i + 1) + '_color" value = "  色  " onclick = "gen_message(this)" disabled> <input type = "button" id = "P2_' + String(h_i + 1) + '_number" value = " 数字 " onclick = "gen_message_order(this, received_data)">';
			}else{
				P2_buttons[h_i].innerHTML = '<input type = "button" id = "P2_' + String(h_i + 1) + '_color" value = "  色  " onclick = "gen_message_order(this, received_data)"> <input type = "button" id = "P2_' + String(h_i + 1) + '_number" value = " 数字 " onclick = "gen_message(this)" disabled>';
			}
		}
		else {
